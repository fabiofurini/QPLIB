...
\Leo{\sout{Following the remark of one of our anonymous referees,} We note that, although the left-to-right progression in the above text marks ``harder problems'', this does \textit{not} mean that \textit{every} instance of a hard problem is itself ``difficult to solve''. As is well known, problems as infinite collections of instances; and it is always possible to find subclasses that can be solved efficiently.}
