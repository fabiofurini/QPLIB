...
the largest\todo{AG: a large?} possible set of interested researchers and practitioners.
...
In addition to spontaneous contribution we analyzed the other generic
libraries of instances available  on the internet and containing QP
...
Note that this might occasionally lead us to classify some instances that have conic constraints as non-convex ones, although their feasible region is in fact convex---fortunately, only some solvers are capable of properly exploiting this property.
 All this information allowed us to analyze the gathered instances and to perform the
...
the instances with a quadratic objective function, ordered by 
 percentage of ``problematic'' \textcolor{red}{(defined using a tolerance of XXX)} \todo{please define the tolerance used to identify the ``Problematic'' eigenvalues and change the label of the plot}\ eigenvalues in the Hessian $Q^0$ (vertical axis), by which
