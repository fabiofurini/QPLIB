...
information allowed us to analyse the gathered instances and to perform the
...
\begin{itemize}
 \item \emph{First Instances Filter.}\\
       ...
       We then filtered according to a relative
 ...
 \item \emph{Second Instances Filter.}\\
       ...
       able to tackle all QP categories\todo{do you mean a complete solver for QGQ? if so, maybe say so}\xspace with a time limit of 120 seconds; all the
...
instances\todo{Maybe one could put the numbers for the original set of instances
into Tables \ref{tab:DD} and \ref{tab:CC}, too?}\xspace among the different categories. Indeed, the discrete categories
...
Figure~\ref{fig:pic_conv_constr} shows the instances with at least one quadratic constraint sorted according to the number of quadratic constraints (vertical axis). For each instance we report the total number of constraints with a ``$+$''
and the total number of nonconvex quadratic constraints
with a ``$\times$''.

\begin{figure}\centering
  \includegraphics[width=0.85\textwidth]{pic_quad_conv_vs_nonconv.png}
  \todo[inline]{horizontal axis label could say ``Instances with at least one quadratic constraint''}
...
the percentage of negative eigenvalues\todo{Was there some tolerance applied on when an eigenvalue is considered negative? $-10^{-9}$, $-10^{-6}$, etc.?} in the $Q^0$ matrix (vertical axis). The instances are mostly
clustered around two values. About 25\% of the instances are convex\todo{so one also restrict to linear constraints, or may only the objective be convex?},
i.e., they have 0\% of negative eigenvalues. Among the others, a vast
majority has around 50\% of negative eigenvalues. However, instances
with high or low percentages of negative eigenvalues are present, too.

\begin{figure}\centering
  \includegraphics[width=0.85\textwidth]{pic_neg_eig.png}
  \todo[inline]{horizontal axis label should say ``Instances with quadratic objective function''}
