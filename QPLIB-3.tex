...
\multirow{6}*{Quadratic}
          & \multirow{3}*{Binary}
                    & None      &   23\\[1.2 ex]  % there are actually QBB due to the {0,1} bounds on the binary variables
          &         & Linear    &  59\\[1.2 ex]
          &         & Quadratic &   5 \\[1.2 ex]
\cmidrule(lr){2-4}
          & \multirow{2}*{Mixed}
...
%
Beyond all instances in GAMS and \texttt{.lp} format, the website provides a rich set of metadata for each
...
by color.  Figure... shows an example for instance QPLIB... \todo{add figure}
