...
\textcolor{green}{Box constraints are not counted as constraints.} \todo{Ambros: From text, tables, and Figure~1 did I correctly infer that variable bounds are not counted as constraints?  The 23 ``none'' in the discrete table seem to be the blue crosses on the x-axis, but then where do the 3 red ``+'' on the x-axis come from?  Are these continuous instances with box constraints and some boxes are infinite?  Should there be a ``none'' in the continuous table?}
The figure shows
