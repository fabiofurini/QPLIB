...
In comparison to a more \emph{high level} and human readable format like \texttt{.gms}, the new
...
%
\begin{itemize}
 \item percentage of positive and negative eigenvalues of the Hessian $Q^0$;
       % (\emph{\% neg eig}),
 %
 \item density of the Hessian $Q^0$ (number of nonzero entries divided by the total
...
The instances of QPLIB are publicly accessible at the website \url{http://qplib.zib.de}, which was created by extending
scripts and tools initially developed for MINLPLib\,2~\cite{Vigerske2014}.
%
Beyond all instances in GAMS, \texttt{.lp}, AMPL (\texttt{.mod}), and \texttt{.qplib} format, the website provides a rich set of metadata for each
...
\caption{Example for the sparsity pattern of the Jacobian of the constraint functions (left) and
  of the Hessian of the Lagrangian function (right) for instance QPLIB\_2967.
